...
from decision_transducer.evaluation.evaluate_episodes import evaluate_episode, evaluate_episode_goal
...
def experiment(
    ...
    # stats to gather scope of critic's value
    stats = []
    ...
    for iter in range(variant['max_iters']):
        ...
        logs = trainer.train_iteration(num_steps=variant['num_steps_per_iter'], iter_num=iter+1, plot_dict = plot, print_logs=True, goal = True)
        if log_to_wandb:
            wandb.log(logs)
...
if __name__ == '__main__':
    ...
    experiment(f'DTD-Critic-{batch}-10k-warmdecay-{lr}-lr-prenorm-3layerencoders-{ norm_prefix }ln-postnorm-emb-{modality_emb}', variant=vars(args))